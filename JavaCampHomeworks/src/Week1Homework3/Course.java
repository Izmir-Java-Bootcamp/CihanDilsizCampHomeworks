...
public class Course {
		...
		if(getNumberOfStudent()>0) {
			...
			for(int i=0;i<getNumberOfStudent();i++) {
				if(students[i].getId()==student.getId()) {
					System.out.println("Aynı Öğrenci İki kez eklenemez");
					return false;
				}
			}
		}
	...
	public void list() {
		if(this.getNumberOfStudent()==0) {
			System.out.println("Bu sınıfta hiçbir öğrenci bulunmuyor");
		}else {
			System.out.println(this.name+" Dersi Öğrencileri");
			for( int i=0;i<getNumberOfStudent();i++) {
